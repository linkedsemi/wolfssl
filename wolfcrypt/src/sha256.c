...
#undef WOLFSSL_USE_ESP32_CRYPT_HASH_HW
    ...
    */
    ...
    !defined(CONFIG_WOLFSSL_LINKEDSEMI_OTBN_SHA224_SHA256_ALT))
...
#define XTRANSFORM_LEN(...) inline_XTRANSFORM_LEN(__VA_ARGS__)
    ...
    */
    ...
    #define SW_XTRANSFORM(S, D)         Transform_Sha256((S),(D))
        ...
        word32 len)
    {
        ...
        if (sha256->buffLen > 0) {
            ...
            if (sha256->buffLen == WC_SHA256_BLOCK_SIZE) {
                ...
                /* Always SW */
                ret = SW_XTRANSFORM(sha256, (const byte*)local);
            ...
            (defined(HAVE_INTEL_AVX1) || defined(HAVE_INTEL_AVX2)))
        {
            while (len >= WC_SHA256_BLOCK_SIZE) {
                ...
                ret = SW_XTRANSFORM(sha256, (const byte*)local32);
    ...
    int wc_Sha256Update(wc_Sha256* sha256, const byte* data, word32 len)
    {
        ...
        ret = Sha256Update((wc_Sha256*)sha256, data, len);
        return ret;
    }
    ...
    int wc_Sha256Final(wc_Sha256* sha256, byte* hash)
    {
        ...
        wc_InitSha256_ex(sha256, NULL, INVALID_DEVID);
        InitSha256(sha256);
        return ret;
    }
    ...
    int wc_InitSha256_ex_dma(wc_Sha256* sha256, void* heap, int devId)
    {
        ...
        XMEMSET(sha256, 0, sizeof(wc_Sha256));
        wc_LSSHA_SHA256_Init_dma(&sha256->lsCtx);
        return 0;
    }

    int wc_Sha256Update_dma(wc_Sha256* sha256, const byte* data, word32 len)
    {
        int ret = 0;
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = wc_LS_Hash_Update_dma(&sha256->lsCtx, data, len);
            wolfSSL_CryptHwMutexUnLock();
        }
        return ret;
    }

    int wc_Sha256Final_dma(wc_Sha256* sha256, byte* hash)
    {
        int ret = 0;
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = wc_LS_Hash_Final_dma(&sha256->lsCtx, hash);
            wolfSSL_CryptHwMutexUnLock();
        }
...
/* implemented in wolfcrypt/src/port/Renesas/renesas_rx64_hw_sha.c */
    ...
    int wc_Sha224Final(wc_Sha224* sha224, byte* hash)
    {
        ...
        wc_InitSha224_ex(sha224, NULL, INVALID_DEVID);
        InitSha224(sha224);
        return ret;
    }
    ...
    int wc_InitSha224_ex_dma(wc_Sha224* sha224, void* heap, int devId)
    {
        ...
        XMEMSET(sha224, 0, sizeof(wc_Sha224));
        wc_LSSHA_SHA224_Init_dma(&sha224->lsCtx);
        return 0;
    }

    int wc_Sha224Update_dma(wc_Sha224* sha224, const byte* data, word32 len)
    {
        int ret = 0;
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = wc_LS_Hash_Update_dma(&sha224->lsCtx, data, len);
            wolfSSL_CryptHwMutexUnLock();
        }
        return ret;
    }

    int wc_Sha224Final_dma(wc_Sha224* sha224, byte* hash)
    {
        int ret = 0;
        ...
        ret = wolfSSL_CryptHwMutexLock();
        if (ret == 0) {
            ret = wc_LS_Hash_Final_dma(&sha224->lsCtx, hash);
            wolfSSL_CryptHwMutexUnLock();
        }
...
int wc_Sha224_Grow(wc_Sha224* sha224, const byte* in, int inSz)
{
    ...
    int wc_Sha224GetHash(wc_Sha224* sha224, byte* hash)
    {
        ...
        if (ret == 0) {
            ret = sw_Sha224Final(tmpSha224, hash);
    ...
    int wc_Sha256GetHash(wc_Sha256* sha256, byte* hash)
    {
        ...
        if (ret == 0) {
            ret = sw_Sha256Final(tmpSha256, hash);
